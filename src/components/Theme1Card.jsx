    ...
    <div className={`flex flex-col md:flex-row gap-6 md:gap-12 w-full max-w-[90vw] md:max-w-[1200px] mx-auto items-stretch font-['Outfit'] transition-all duration-700 flex-1 md:flex-none md:h-[600px] xl:h-[650px]`}>
      ...
      >
        ...
        <div>
          <div className="flex justify-between items-start gap-4 z-20 relative">
            <div className="flex-1 min-w-0 pr-16 md:pr-0">
              ...
              <h1 className={`${island.name.length > 8 ? 'text-2xl md:text-3xl' : 'text-3xl md:text-4xl'} font-black mb-3 md:mb-4 uppercase tracking-tighter drop-shadow-sm leading-tight truncate`}>{island.name}</h1>
            </div>
        ...
        <div className="p-5 lg:p-6 rounded-3xl transition-all duration-500 cursor-pointer shadow-xl mt-auto shrink-0 flex flex-col gap-3" 
          ...
          <p className="text-base font-medium opacity-90 line-clamp-2 leading-relaxed">{formatBoldText(island.story?.description)}</p>
        </div>
      </div>
    </div>
  );
